...
> **РЕАЛИЗОВАНО (PR-4, ultracode-армия 15 агентов → SHIP-LEAN, оба судьи + 4 скептика):** **МЕСТО ПЕРЕНЕСЕНО worker → GPU-lane.** У воркера НЕТ аудио (качает только raw JSON c R2), а forced-alignment нужна и волна, и CTC-модель — оба живут на GPU-lane. Реализовано как `services/gpu-gigaam/fliphouse_gigaam/align.py` (`realign_payload` + `_realign_segment` + `_sanitize_spans` + `forced_align_enabled`/`resolve_align_fn`; реальный `_default_ctc_align` = GigaAM `v3_ctc` + `torchaudio.functional.forced_align`/`merge_tokens`, BSD-2, `# pragma: no cover`), проводка одной строкой в `modal_app._transcribe` после `payload_from_longform` (повторно использует уже декодированный 16k wav, без второго decode). **Контракт `RawPayload` НЕ изменён, воркер не тронут.** OFF-default (`ASR_FORCED_ALIGN_ENABLED` unset) → `realign_payload` возвращает ТОТ ЖЕ объект → байт-идентичные callback-байты/`word_segments`/`cascade`/goldens. **HIGH-фиксы скептиков, вшитые в спеку:** (1) wall-clock budget (`ALIGN_WALL_BUDGET_S`, инжектируемые `now_fn`/`budget_s`) — при исчерпании остальные сегменты держат RNN-T (partial progress, Modal не убивает контейнер mid-pass); (2) поправка: word-тайминги — это вход НЕ только субтитров, но и boundary-снаппера (`score.py → recall/segmenter refine_boundaries`), поэтому ON может сдвинуть выбор клипов → **live-валидация ДОЛЖНА включать OFF-vs-ON diff `clips.json` на `tinkov-plata.mp4` через P3-eval-gate, не только caption frame-drift.** Кламп слов в окно сегмента нейтрализует cascade-drift при `duration==0`. Гейты: gpu-gigaam 97 passed, 100% cov, ruff/black 0. **Остаток founder-gated до флипа env:** live-калибровка `FRAME_SEC` (стрид CTC-субсэмплинга) + emission/tokenizer GigaAM-CTC head + проверка <100мс дрейфа.
...
> **РЕАЛИЗОВАНО (PR-5, ultracode-армия 15 агентов → SHIP-LEAN, но adversarial опрокинул `-threads`).** **`-threads` ОТВЕРГНУТ (CRITICAL скептика, подтверждён по коду):** render-цикл уже фанаутит `MAX_RENDER_WORKERS=4` нативных энкодера через `strict_ordered_threadpool_map` на 2-vCPU Railway-боксе (`concurrency.py:31`, докстринг «a 30-clip job must not fork 30 native encoders on a 2-vCPU box») — то есть бокс уже насыщен на уровне КЛИПОВ (правильный режим для batch-throughput). Intra-clip `-threads 2` = 4×2=8 потоков на 2 ядра → ускорения single-clip ноль (нет свободных ядер), только slice-потеря компрессии OpenH264 + риск качества + context-switch → net throughput zero-to-NEGATIVE. GOP/maxrate/bufsize — это rate-control/каденция, НЕ speed-рычаги (тратят SSIM-бюджет ради нуля). **Шипнута только реальная ценность: байт-идентичная DRY-экстракция** `_video_encoder_args(bitrate)` + `_audio_encoder_args()` из 3 argv-билдеров (`_build_render_argv`/`_build_video_render_argv`/`_build_concat_mux_argv` ass-ветка) в ОДНО место — убирает трипликацию и **это ровно тот шов, где B2 подменит кодек на NVENC/VideoToolbox** (libopenh264 — гарантированный default+fallback). `bitrate` остаётся параметром (concat-mux передаёт свой). `-c:v copy` no-reencode-ветка не тронута. Доказательство byte-identity: 16 существующих argv/filtergraph-goldens зелёные без изменений + 2 новых pin-теста (`test_video_encoder_args_pins_libopenh264_block`/`test_audio_encoder_args_pins_aac_block`). Гейты: ai-worker **1232 passed, 8 skipped, 100% cov, ruff/black 0**. **Реальные speed-рычаги энкод-стороны = B2 (HW-encoder) + отдельный follow-up: filter-стадия (scale/gblur/libass) авто-тредится на ХОСТ-nproc, не на cgroup-квоту → oversubscribe под K параллельными клипами (reliability-тикет, не B1).**
