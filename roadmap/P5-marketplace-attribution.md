...
4. **Payouts/attribution на cliq-субстрате** — Link → Conversion → Commission: трекинг-ссылка на оффер, конверсия (просмотр/показ/клик), детерминированный расчёт комиссии (Function/Condition/Effect), начисление в ledger, выплата креатору в USDT через `PaymentProvider.createPayout` (свой TRON on-chain transfer, `tronweb` — аналог Stripe Connect).
...
# tronweb + TRON-узел/TronGrid (свой on-chain приёмник, PaymentProvider) — для debit с баланса + выплаты createPayout (TRC-20 transfer).
...
# фикстуры подтверждённых TRON-транзакций — в fixtures/payments/*.json (наш on-chain watcher парсит их).
# На ЧЕКПОИНТЕ — реальный депозит/выплата на TRON testnet (Nile/Shasta).
```
...
| ЧП-9 | 5.24 | Payout/settlement: расчёт всех payout-моделей + выплата креатору в USDT через `PaymentProvider.createPayout` (свой TRON, `tronweb`), monthly settle |
...
- **Свой TRON on-chain** → mock через интерфейс `PaymentProvider` (подменённая реализация) + фикстуры подтверждённых TRON-tx (записаны в `fixtures/payments/*.json`). Идемпотентность проверяется повторной обработкой одной и той же tx (`txid`).
...
> через `PaymentProvider.debit`. Никакого делегирования дедупа третьей стороне.
...
- **Репозитории/команды:** — (`tronweb`/выплаты подключаются на billing-прогоне, не здесь; запись в ledger — чистый Drizzle).
...
### Шаг 5.24 — Выплата креатору в USDT через `PaymentProvider.createPayout` (свой TRON) 🛑 ЧЕКПОИНТ
- **Цель / DoD:** `executePayout(payout)` через `PaymentProvider.createPayout` (**TRC-20 USDT transfer** `tronweb` на USDT-адрес креатора с payout-hot-кошелька — аналог Stripe Connect transfer), идемпотентно по `payout.id` (наш idempotency key; не шлём второй transfer, пока первый не зафейлился). Обновляет `payouts.status pending→broadcast→paid`/`failed`. Наш on-chain watcher по `txid` финализирует (≥N подтверждений).
- **Репозитории/команды:** `tronweb` + payout-hot-кошелёк (ключ из KMS), TRON-узел/TronGrid. Фикстуры TRON-tx для тестов.
- **Тесты СНАЧАЛА** (`src/payout/executePayout.test.ts`, integration + mocked `PaymentProvider`):
  ...
  - `test('повторный executePayout НЕ шлёт вторую выплату')` — наша guard по payout.id.
  - `test('подтверждённая on-chain tx (≥N) переводит payout→paid по txid')`.
  - `test('зафейленный/реверт transfer переводит payout→failed с reason')`.
  - `test('payout без привязанного USDT-адреса креатора → held reason=no_payout_address')`.
  - `test('недостаточный баланс payout-hot-кошелька → held reason=insufficient_hot_balance')`.
- **Реализация:** `src/payout/executePayout.ts` (`tronweb` transfer, ключ из KMS), финализация в on-chain watcher по `txid`.
- **✅ Готово когда:** тесты зелёные; идемпотентность выплаты доказана; ключ payout-кошелька только из KMS, не в коде/БД.
- **🛑 ЧЕКПОИНТ ЧП-9:** основатель ревьюит выплаты: корректна ли сумма USDT, идемпотентна ли выплата, как обрабатывается отсутствие крипто-адреса, безопасен ли payout-hot-кошелёк (KMS, лимиты), confirmations-гейт. Может изменить payout-расписание, KYC/AML-гейт, hot/cold-политику.
...
- [ ] **Payout:** monthly settlement → выплата креатору в USDT через `PaymentProvider.createPayout` (свой TRON, `tronweb`), идемпотентно по `payout.id`/`txid`, финализация по on-chain подтверждениям, KYC/AML + крипто-адрес-гейт, payout-hot-key в KMS (ЧП-9).
