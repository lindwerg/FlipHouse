...
>
> Учётный движок (Link → Conversion → Commission, Function/Condition/Effect) **реимплементируется clean-room по блюпринту `org-quicko/cliq`** (репозиторий без лицензии — копировать код НЕЛЬЗЯ, см. doc 01 §1 «Правовые красные флаги»). Поверх Stripe-подписок из Phase 1 добавляем **Stripe usage/metered billing** (per-clip / per-render / CPM). Атрибуция показов/CPM — **v1 по doc 03 §5**: трекинг-ссылки + платформенная аналитика через creator-OAuth, с честными ограничениями (view-attribution, не viewability).
>
...
4. **Payouts/attribution на cliq-субстрате** — Link → Conversion → Commission: трекинг-ссылка на оффер, конверсия (просмотр/показ/клик), детерминированный расчёт комиссии (Function/Condition/Effect), начисление в ledger, выплата креатору через Stripe Connect.
5. **Stripe usage/metered billing** — события usage (per-clip / per-render / CPM) → Stripe Billing Meters, идемпотентно (no double-charge на ретрае).
...
**Definition of Done фазы:** проходит сквозной e2e «advertiser posts offer → creator applies → accepted → clip rendered with that offer's banner → conversion recorded → payout accrued → Stripe metered», метеринг идемпотентен (нет двойного списания на ретрае), расчёт выплат покрыт unit-тестами на каждую payout-модель (`cpm` / `per_1k_views` / `flat` / `hybrid`), глобальное покрытие ≥ 80%.
...
|---|---|---|
| **Phase 0** (каркас/инфра) | Railway-проект, `web` (SaaS-Boilerplate + Drizzle + Stripe + auth/RBAC), `Postgres`, `Redis`, `bullmq-worker` | На этом стоит вся БД маркетплейса, очередь начислений, Stripe-клиент |
| **Phase 1** (клиппинг-движок MVP) | `POST /clips` → `GET /clips/{job_id}`, рендер 9:16, `bullmq-worker`, Stripe **подписки** | Метеринг (usage) надстраивается над подписками; рендер-джоба расширяется оффер-баннером |
...
# 3. nextjs/saas-starter — уже завендорен в Phase 0/1 как основа web. Здесь только сверяемся со Stripe-webhook/Drizzle-слоем.
#    (если ещё нет в репо:)
...
pnpm add stripe@latest                      # Stripe Billing Meters (usage/metered) + Connect payouts
...
pnpm add -D stripe-event-types@latest                  # типы вебхуков
# stripe CLI (для локального webhook-forward + фикстур событий метеринга):
#   brew install stripe/stripe-cli/stripe   (или скачать бинарь)
```
...
| ЧП-7 | 5.19 | Stripe usage/metered: события usage идемпотентны, нет двойного списания на ретрае |
| ЧП-8 | 5.22 | Attribution v1: creator-OAuth метеринг по дельтам + аудит, честные ограничения зафиксированы |
| ЧП-9 | 5.24 | Payout/settlement: расчёт всех payout-моделей + Stripe Connect выплата, monthly settle |
...
- **Stripe** → mock через `stripe` SDK с подменённым `httpClient` + фикстуры событий из `stripe trigger` (записаны в `fixtures/stripe/*.json`). Идемпотентность проверяется повторной доставкой одного и того же `event.id`.
...
- **Реализация:** клонировать вендор; написать ADR с таблицей маппинга cliq-сущность → наша таблица → отличия (главное: «cliq останавливается на accrual, не двигает деньги — мы добавляем payout/Stripe Connect»).
...
- **Репозитории/команды:** `pnpm add drizzle-orm`, `pnpm add -D drizzle-kit`. Сверка со Stripe/Drizzle-слоем `vendor/saas-starter`.
- **Тесты СНАЧАЛА** (`src/db/schema/marketplace.test.ts`, integration против `fliphouse_test`):
  ...
  - `test('usage_events уникален по stripe_idempotency_key')` — основа no-double-charge.
  - `test('FK application.offer_id → offers.id с ON DELETE RESTRICT')`.
- **Реализация:** `src/db/schema/marketplace.ts` (все таблицы; деньги — `numeric(18,4)`; статусы — pgEnum; `tracking_links.code` unique; `usage_events.stripeIdempotencyKey` unique). `drizzle-kit generate` → `drizzle/00XX_p5_marketplace.sql`. Скрипт миграции test-БД в `vitest.setup.ts`.
...
## Блок F — Stripe usage / metered billing (ЧП-7)

### Шаг 5.18 — Stripe Billing Meters: usage event emitter (идемпотентный)
- **Цель / DoD:** `emitUsage(meterEvent)` шлёт `stripe.billing.meterEvents.create` с **`identifier` = детерминированный idempotency-key** (per-clip / per-render / CPM). Перед отправкой пишет `usage_events` row (unique `stripe_idempotency_key`) — `INSERT ... ON CONFLICT DO NOTHING`; если конфликт → событие уже отправлено → skip (no double-charge).
- **Репозитории/команды:** `pnpm add stripe`, `pnpm add -D stripe-event-types`. `stripe CLI` для фикстур.
- **Тесты СНАЧАЛА** (`src/billing/usage/emitUsage.test.ts`, integration + mocked Stripe):
  - `test('emitUsage(per_render) шлёт meterEvent с детерминированным identifier')`.
  - `test('повторный emitUsage того же события НЕ шлёт второй раз в Stripe')` — мок считает вызовы == 1.
  - `test('конкурентные emitUsage (та же джоба ретраится) → один Stripe-вызов')` — unique-constraint держит.
  - `test('Stripe 5xx → ретрай с backoff, но identifier тот же (Stripe дедупит на своей стороне)')`.
  - `test('три типа usage (per_clip/per_render/cpm) мапятся на правильные meter event_name')`.
- **Реализация:** `src/billing/usage/emitUsage.ts` (Stripe Billing Meters API), `src/billing/usage/meterKeys.ts` (детерминированная генерация identifier: `${type}:${input_hash}:${unit}`).
- **✅ Готово когда:** 5 тестов зелёные; двойная защита (наша таблица + Stripe identifier) доказана.
- **Commit:** `feat: idempotent Stripe metered usage events (per-clip/render/cpm)`
...
- **🛑 ЧЕКПОИНТ ЧП-7:** основатель ревьюит метеринг: события usage идемпотентны, нет двойного списания на ретрае вебхука/джобы, корректный маппинг типов на Stripe meters. Может изменить, что именно метрируется и по какой цене.
...
### Шаг 5.24 — Stripe Connect payout (выплата креатору) 🛑 ЧЕКПОИНТ
- **Цель / DoD:** `executePayout(payout)` через Stripe Connect (`transfers.create` на connected account креатора), идемпотентно по `payout.id` (Stripe idempotency key). Обновляет `payouts.status pending→paid`/`failed`. Webhook `transfer.paid`/`transfer.failed` финализирует.
- **Репозитории/команды:** `stripe` SDK (Connect). `stripe CLI` для webhook-фикстур.
- **Тесты СНАЧАЛА** (`src/payout/executePayout.test.ts`, integration + mocked Stripe):
  - `test('executePayout шлёт transfer на connected account с idempotency key = payout.id')`.
  - `test('повторный executePayout НЕ шлёт второй transfer')` — Stripe idempotency + наша guard.
  - `test('transfer.paid вебхук переводит payout→paid')`.
  - `test('transfer.failed вебхук переводит payout→failed с reason')`.
  - `test('payout без онбордженного Connect-аккаунта → held reason=no_connect_account')`.
  - `test('webhook с невалидной подписью → 400, состояние не меняется')` — security (common/security).
- **Реализация:** `src/payout/executePayout.ts`, `src/app/api/webhooks/stripe-connect/route.ts` (verify signature → finalize).
...
- **🛑 ЧЕКПОИНТ ЧП-9:** основатель ревьюит выплаты: корректна ли сумма, идемпотентна ли выплата, как обрабатывается отсутствие Connect-аккаунта, верифицируются ли вебхуки. Может изменить payout-расписание, KYC-гейт, валютную логику.
- **Commit:** `feat: Stripe Connect payout execution + webhook finalization`
...
- **Цель / DoD:** один Playwright-сценарий проходит весь цикл: advertiser постит оффер → активируется → creator применяет к клипу → accepted → клип рендерится с этим баннером (golden-проба) → conversion записана → payout начислен → Stripe metered зафиксирован. Это «#1 правило основателя» в исполнении.
- **Репозитории/команды:** Playwright + render-воркер + mocked Stripe/platform APIs.
- **Тесты СНАЧАЛА** (`e2e/marketplace-flow.spec.ts`, Playwright):
  - `test('full flow: post offer → apply → accept → render → conversion → payout')` — единый сценарий со всеми этапами, ассертит на каждом стыке (offer active, application accepted, impression_unit rendered с правильным offerId, conversion с delta, ledger accrual, payout pending, Stripe meter event записан).
...
- [ ] **Stripe usage/metered:** события usage (per-clip/per-render/cpm) идемпотентны — **нет двойного списания на ретрае** (двойная защита: наша unique-таблица + Stripe identifier) (ЧП-7).
...
- [ ] **Payout:** monthly settlement → Stripe Connect выплата, идемпотентно, вебхук-подпись верифицируется до мутации, KYC/Connect-гейт (ЧП-9).
