...
>
> Учётный движок (Link → Conversion → Commission, Function/Condition/Effect) **реимплементируется clean-room по блюпринту `org-quicko/cliq`** (репозиторий без лицензии — копировать код НЕЛЬЗЯ, см. doc 01 §1 «Правовые красные флаги»). Поверх подписок из Phase 1 (через `PaymentProvider`) добавляем **metered-биллинг как наш собственный `usage_events`-ledger** (per-clip / per-render / CPM) → периодическое списание через `PaymentProvider.chargeRecurring`. У РФ-операторов нет Stripe-style Billing Meters, поэтому идемпотентность держим **на нашей стороне** (unique-ключ), не делегируем оператору. Атрибуция показов/CPM — **v1 по doc 03 §5**: трекинг-ссылки + платформенная аналитика через creator-OAuth, с честными ограничениями (view-attribution, не viewability).
>
...
4. **Payouts/attribution на cliq-субстрате** — Link → Conversion → Commission: трекинг-ссылка на оффер, конверсия (просмотр/показ/клик), детерминированный расчёт комиссии (Function/Condition/Effect), начисление в ledger, выплата креатору через `PaymentProvider.createPayout` (РФ-оператор: ЮKassa Выплаты / CloudPayments Pay-out / Т-Банк — аналог Stripe Connect).
...
# SDK выбранного РФ-оператора (TBD: ЮKassa/CloudPayments/Т-Банк) — для PaymentProvider.chargeRecurring + createPayout.
# Конкретный пакет добавляется при выборе оператора перед кодом блоков F/H. Metered-биллинг — наш usage_events-ledger, не Stripe Meters.
...
# типы/фикстуры вебхуков оператора — TBD при выборе оператора (записываем нормализованные фикстуры уведомлений в fixtures/payments/*.json).
# Локальный webhook-forward — средствами выбранного оператора (личный кабинет / их CLI), доставка в e2e на ЧЕКПОИНТЕ.
```
...
| ЧП-9 | 5.24 | Payout/settlement: расчёт всех payout-моделей + выплата через `PaymentProvider.createPayout` (РФ-оператор), monthly settle |
...
- **Платёжный оператор** → mock через интерфейс `PaymentProvider` (подменённая реализация) + нормализованные фикстуры уведомлений (записаны в `fixtures/payments/*.json`). Идемпотентность проверяется повторной доставкой одного и того же `eventId`.
...
> У РФ-операторов нет Stripe-style Billing Meters. Поэтому metered-биллинг — это **наш** ledger
> `usage_events` (источник истины идемпотентности), из которого периодически формируется списание
> через `PaymentProvider.chargeRecurring`. Никакого делегирования дедупа оператору.

### Шаг 5.18 — usage event recorder (идемпотентный, наш ledger)
- **Цель / DoD:** `recordUsage(usageEvent)` пишет `usage_events` row с **`paymentIdempotencyKey` = детерминированный idempotency-key** (per-clip / per-render / CPM) через `INSERT ... ON CONFLICT DO NOTHING`; если конфликт → событие уже учтено → skip (no double-charge). Списание агрегата — отдельным billing-прогоном через `PaymentProvider.chargeRecurring`.
- **Репозитории/команды:** — (SDK оператора подключается на billing-прогоне, не здесь; запись в ledger — чистый Drizzle).
...
### Шаг 5.24 — Выплата креатору через `PaymentProvider.createPayout` (РФ-оператор) 🛑 ЧЕКПОИНТ
- **Цель / DoD:** `executePayout(payout)` через `PaymentProvider.createPayout` (выплата на реквизиты/способ креатора: ЮKassa Выплаты / CloudPayments Pay-out / Т-Банк — аналог Stripe Connect transfer), идемпотентно по `payout.id` (наш idempotency key + idempotency-механизм оператора, если есть). Обновляет `payouts.status pending→paid`/`failed`. Webhook оператора (`payout.succeeded`/`payout.failed`) финализирует.
- **Репозитории/команды:** SDK выбранного оператора (выплаты). Фикстуры уведомлений оператора для webhook-тестов.
- **Тесты СНАЧАЛА** (`src/payout/executePayout.test.ts`, integration + mocked `PaymentProvider`):
  - `test('executePayout шлёт createPayout на реквизиты креатора с idempotency key = payout.id')`.
  - `test('повторный executePayout НЕ шлёт вторую выплату')` — наша guard + idempotency оператора.
  - `test('payout.succeeded вебхук переводит payout→paid')`.
  - `test('payout.failed вебхук переводит payout→failed с reason')`.
  - `test('payout без онбордженных реквизитов выплаты → held reason=no_payout_account')`.
...
- **🛑 ЧЕКПОИНТ ЧП-9:** основатель ревьюит выплаты: корректна ли сумма, идемпотентна ли выплата, как обрабатывается отсутствие реквизитов выплаты, верифицируются ли вебхуки. Может изменить payout-расписание, KYC-гейт (самозанятые/физлица), валютную логику.
- **Commit:** `feat: creator payout via PaymentProvider.createPayout + webhook finalization`
...
- [ ] **Payout:** monthly settlement → выплата через `PaymentProvider.createPayout` (РФ-оператор), идемпотентно, вебхук-подпись верифицируется до мутации, KYC/реквизиты-гейт (ЧП-9).
