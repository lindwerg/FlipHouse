...
# SDK ЮKassa (первая конкрет-реализация PaymentProvider) — для chargeRecurring + ЮKassa Выплаты (createPayout).
...
# фикстуры вебхуков ЮKassa — нормализованные уведомления в fixtures/payments/*.json.
# Локальный webhook-forward — средствами ЮKassa (личный кабинет / HTTP-уведомления), доставка в e2e на ЧЕКПОИНТЕ.
```
...
- **Цель / DoD:** `executePayout(payout)` через `PaymentProvider.createPayout` (**ЮKassa Выплаты** на карту/СБП/кошелёк креатора — аналог Stripe Connect transfer), идемпотентно по `payout.id` (наш idempotency key + idempotency-ключ ЮKassa). Обновляет `payouts.status pending→paid`/`failed`. Webhook ЮKassa (`payout.succeeded`/`payout.canceled`) финализирует.
- **Репозитории/команды:** SDK ЮKassa (Выплаты). Фикстуры уведомлений ЮKassa для webhook-тестов.
