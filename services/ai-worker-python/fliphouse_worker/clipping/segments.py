...
must TRACK a speaker for part of its span and show the WHOLE frame (blur-pad fit)
...
CROP↔BLURPAD per keyframe so a single dropped/extra face never flips the mode;
short segments are merged and transition boundaries snap to scene cuts. PURE.
"""
...
N_DROP_SAMPLES: int = 3  # 1.5s @2Hz of "no face" before CROP→BLURPAD (sticky to tracking)
N_ACQUIRE_SAMPLES: int = 2  # 1.0s @2Hz of "face" before BLURPAD→CROP (quicker to re-acquire)
...
def resolve_mode_timeline(
    ...
    *,
    ...
    """Per-keyframe TRACK/GENERAL → debounced per-keyframe CROP_MODE/BLURPAD_MODE.

    The state is SEEDED from ``keyframes[0]`` (TRACK→CROP, GENERAL→BLURPAD) so a
    talking-head clip never opens with a spurious blur-pad intro. Asymmetric
    hysteresis applies only to transitions: CROP→BLURPAD after ``n_drop``
    consecutive GENERAL keyframes; BLURPAD→CROP after ``n_acquire`` consecutive
    TRACK keyframes. Empty input → empty tuple. PURE.
    """
    if not keyframes:
        return ()
...
def build_render_segments(
    ...
    *,
    ...
    No keyframes → ONE blur-pad segment (fail-safe). A single-mode trajectory →
    ...
    → short-segment merge → boxes (TRACK→speaker column on the run's median
    centre; GENERAL→full-frame blur-pad). PURE.
    """
    kfs = traj.keyframes
    ...
    for s in segs:
        if s["mode"] == BLURPAD_MODE:
            box = full_frame
        else:
            centre = median(s["centers"]) if s["centers"] else None
            box = compute_crop_box(traj.source_width, traj.source_height, centre)
        out.append(RenderSegment(s["start"], s["end"], box))
    return tuple(out)
