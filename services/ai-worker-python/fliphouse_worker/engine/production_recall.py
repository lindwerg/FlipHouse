...
production, not dormant:
    ...
    score_handler → _default_score_clips → build_phrase_anchored_recall_fn
        → recall_candidates(align_fn=<RapidFuzz>, punct_fn=<RU restorer>, word_segments=…)
...
— whose own ``sent_end`` snapper still uses the license-clean pause/discourse
heuristic (no model weights pulled into the worker, no CC-BY-NC restorer).
