...
def test_segment_to_dict_nests_words():
    seg = Segment(0.0, 2.0, (Word("a", 0.0, 1.0), Word("b", 1.0, 2.0)))
    assert seg.to_dict() == {
        "start": 0.0,
        "end": 2.0,
        "words": [
...
def test_raw_payload_to_dict_matches_receiver_contract():
    payload = RawPayload(
        duration=2.0,
        language="ru",
        segments=(Segment(0.0, 2.0, (Word("x", 0.0, 2.0),)),),
    )
    assert payload.to_dict() == {
        "duration": 2.0,
        "language": "ru",
        "segments": [
            {"start": 0.0, "end": 2.0, "words": [{"word": "x", "start": 0.0, "end": 2.0}]},
        ],
    }
