...
> **[РЕШЕНИЕ FOUNDER'А · 2026-06-15] STRIPE УБРАН → ЮKassa + подписка с лимитом минут.** Stripe не работает в РФ. Оператор = **ЮKassa** (первая конкрет-реализация vendor-нейтральной абстракции `PaymentProvider`, как `PublishProvider` в P6: `createCheckout`/`chargeRecurring`/`getSubscriptionStatus`/`verifyWebhook`/`createPayout`). Рубли, карты+СБП, автоплатежи, **54-ФЗ чеки**. Webhook ЮKassa НЕ подписан HMAC — подлинность через IP-allowlist + re-fetch `GET /payments/{id}`. **Монетизация = подписка креатора** (рекламодатель платит за офферы отдельно в P5). Тариф лимитирует **минуты исходного видео/мес** (главный драйвер себес — GPU reframe/ASD, ~$0.025/мин): Бесплатно 30 / Старт 990₽·150 / **Актив 1990₽·300** / Студия 4990₽·1000, overage 30₽/мин. Сетка **выверена против Opus Clip** (их Pro = $29·300 мин — наш «Актив» 1:1). Биллинг-состояние — на **`userId`** (таблица `subscription`), НЕ на организации (удалены в P1.11). Metered-биллинг P5 — НЕ Stripe Meters, а наш `usage_events`-ledger. Stripe-кода в репо НЕ было (форк его срезал) — сделан ре-план роадмапа/доков (P1.12/1.13, P5, P7, ROADMAP, docs 00/01/03/05) + чистка локалей; кода ещё нет, P1.12/P1.13 реализуются по TDD после аппрува.
>
...
- ⬜ ЧП F: подписка ЮKassa checkout/оплата + тарифы с лимитом минут + webhook
...
- ⬜ ЧП-9 (5.24): payout через `PaymentProvider.createPayout` (РФ-оператор) + settlement
