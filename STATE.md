...
> **[РЕШЕНИЕ FOUNDER'А · 2026-06-15] STRIPE УБРАН → `PaymentProvider` (РФ-оператор TBD).** Stripe не работает в РФ. Берём РФ платёжного оператора (ЮKassa / CloudPayments / Т-Банк — выбор позже, отдельным решением founder'а перед кодом P1.12). Биллинг спрятан за **vendor-нейтральной абстракцией `PaymentProvider`** (как `PublishProvider` в P6): `createCheckout` / `chargeRecurring` / `getSubscriptionStatus` / `verifyWebhook` / `createPayout`. Stripe-кода в репо НЕ было (форк `ixartz/SaaS-Boilerplate` его срезал) — «удаление» = ре-план роадмапа/доков (P1.12/1.13, P5, P7, ROADMAP, docs 00/01/03/05) + чистка остатка в локалях. Шаги P1.12/P1.13 ещё не реализованы — теперь перепланированы под оператора и переведены с организации на **`userId`** (организации удалены в P1.11). Metered-биллинг P5 — НЕ Stripe Meters, а наш `usage_events`-ledger (идемпотентность на нашей стороне). Сделано ре-планом, кода нет.
>
...
- ⬜ ЧП F: `PaymentProvider` (РФ-оператор) checkout/оплата + webhook
