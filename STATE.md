...
- 🛑 ЧП A: OpenRouter-адаптер (модели/роуты/JSON-схема/fallback) — **код готов (P2.2), ждёт ревью founder'а**. На ревью: слаги моделей по профилям (scoring дёшево / offer_match сильно), формат scoring JSON-схемы (doc 04 §2.4), политика fallback/backoff (429/conn→retry, 402→fatal, ≥500→retry), заголовки атрибуции (`HTTP-Referer`/`X-OpenRouter-Title`). Founder может поменять слаги в `routes.py` и пороги retry.
