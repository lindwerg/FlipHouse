...
- 🛑 ЧП G: **ОСТАНОВ — ЖДУ founder'а** (после P1.16). Задеплоено на staging (живой `https://web-staging-32d77.up.railway.app`); сквозные e2e + hero-drop + визуальная регрессия написаны и зелёные (auth-путь guard-skip в CI — founder гонит вживую); Lighthouse: CWV (LCP/CLS/TBT) в бюджете, First-Load-JS 203kb > 150kb (развилка). Founder вживую: signup→пополнение→дашборд на staging, визуальные снапшоты, Lighthouse-отчёт → решение «промоут staging→production» и закрытие фазы P1. **Не прохожу чекпоинт без явного «го».**
