...
| **Биллинг / выплаты** | `PaymentProvider` — РФ-оператор TBD (ЮKassa/CloudPayments/Т-Банк): эквайринг + выплаты креаторам, бюджеты рекламодателей. Stripe не работает в РФ. | коммерч. | `web` |
...
- Песочница платёжного оператора (РФ, TBD), Clerk auth.
