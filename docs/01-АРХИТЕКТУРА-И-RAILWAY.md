...
| **SaaS-биллинг + auth + RBAC** | `nextjs/saas-starter` | MIT | **Lift + extend** | Кастомный JWT/bcrypt auth, Drizzle-схема, webhook/checkout. Биллинг — за абстракцией `PaymentProvider` → крипто-PSP (USDT-баланс; Stripe-код не используем, не работает в РФ). Расширяем: metered usage (per-clip / per-render / CPM) как наш ledger, роль creator/advertiser в Clerk `publicMetadata` |
