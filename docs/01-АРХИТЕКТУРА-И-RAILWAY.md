...
| **SaaS-биллинг + auth + RBAC** | `nextjs/saas-starter` | MIT | **Lift + extend** | Кастомный JWT/bcrypt auth, Drizzle-схема, Stripe webhook/checkout. Расширяем: Billing Meters (per-clip / per-render / CPM), `accountType` на `teams` |
