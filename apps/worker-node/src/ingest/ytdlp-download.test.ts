...
test('downloadVideo resolves when the injected execFile succeeds', async () => {
  ...
  await expect(
    downloadVideo('https://youtu.be/abc', '/tmp/out.mp4', { execFile, bin: 'yt-dlp', timeoutMs: 1000 }),
...
test('downloadVideo throws a classified IngestDownloadError from execFile stderr', async () => {
  ...
  await expect(
    downloadVideo('https://youtu.be/abc', '/tmp/out.mp4', { execFile }),
...
test('downloadVideo falls back to the default execFile when none is injected', async () => {
  ...
  await expect(
    downloadVideo('https://youtu.be/abc', '/tmp/out.mp4', { bin: 'true', timeoutMs: 5000 }),
  ).resolves.toBeUndefined();
});
